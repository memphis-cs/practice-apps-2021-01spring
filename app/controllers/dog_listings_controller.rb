class DogListingsController < ApplicationController
  before_action :set_dog_listing, only: [:show, :edit, :update, :destroy]

  # GET /dog_listings
  # GET /dog_listings.json
  def index
    @dog_listings = DogListing.all
  end

  # GET /dog_listings/1
  # GET /dog_listings/1.json
  def show
  end
  ...
  # POST /dog_listings
  # POST /dog_listings.json
  def create
    @dog_listing = DogListing.new(dog_listing_params)

    respond_to do |format|
      if @dog_listing.save
        format.html { redirect_to @dog_listing, notice: 'Dog listing was successfully created.' }
  ...
  # PATCH/PUT /dog_listings/1
  # PATCH/PUT /dog_listings/1.json
  def update
    respond_to do |format|
      if @dog_listing.update(dog_listing_params)
        format.html { redirect_to @dog_listing, notice: 'Dog listing was successfully updated.' }
  ...
  # DELETE /dog_listings/1
  # DELETE /dog_listings/1.json
  def destroy
    @dog_listing.destroy
    respond_to do |format|
  ...
  private
    ...
    def dog_listing_params
      params.require(:dog_listing).permit(:name, :breed, :weight, :price, :seller_email)
    end
end
